...
#include "utils/cnpy/include/cnpy.h"
...
void print_embeddings(emb_t* embeddings, unsigned long num_vertices, unsigned long dimension, string fname, bool binary);
...
void print_embeddings(emb_t* embeddings, unsigned long num_vertices, unsigned long dimension, string fname, bool binary){
  if (binary) { 
    ...
    cnpy::npy_save(fname,embeddings,{num_vertices,dimension},"w");
  }
  else { 
    string type="w";
