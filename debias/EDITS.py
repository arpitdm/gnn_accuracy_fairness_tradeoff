'''
Adapted directly from https://github.com/yushundong/EDITS
'''
...
class EDITS(nn.Module):
    ...
    def __init__(self, args, nfeat, node_num, nfeat_out, adj_lambda, layer_threshold=2, dropout=0.1):
        super(EDITS, self).__init__()
        self.x_debaising = X_debaising(nfeat)
        self.layer_threshold = int(layer_threshold)
        self.adj_renew = Adj_renew(node_num, nfeat, nfeat_out, adj_lambda)
        self.fc = nn.Linear(int(nfeat * (layer_threshold + 1)), 1)
        self.lr = args.edits_lr
        ...
        self.optimizer_G = torch.optim.RMSprop(G_params, lr=self.lr, eps=1e-04, weight_decay=args.edits_weight_decay)
        self.optimizer_A = torch.optim.RMSprop(self.fc.parameters(), lr=self.lr, eps=1e-04, weight_decay=args.edits_weight_decay)
